...
*/
	if(s!=NULL)
		s->top = -1;			/*Zaciatok zasobnika je -1*/
	else
		stackError(SERR_INIT);		/*Chyba pri inicializacii*/
}
...
*/
	return !(s->top+1);			/*Na zaciatku je vrchol zasobnika na -1. Ked pripocitam 1 dostanem 0. Negujem.*/
}
...
*/
	return (STACK_SIZE - 1 == s->top);	/*Na konci je vrchol zasobnika  na najvyssom indexe.*/
}
...
*/
	if(!stackEmpty(s))
		*c=s->arr[s->top];		/*Premenna na vrchole zasobnika.*/
	else
		stackError(SERR_TOP);		/*Chyba pri citani top prvku*/
}
...
*/
	if(!stackEmpty(s))
		s->top -= 1;			/*Dekrementujem index vrcholu zasobnika.*/
}
...
*/
	if(!stackFull(s)){
		s->top += 1;			/*Inkrementujem index vrcholu zasobnika.*/
		s->arr[s->top]=c;		/*Vlozim hodnotu.*/
}
	else
		stackError(SERR_PUSH);		/*Chyba pri pushovani*/
}
