...
**/
	tBTNodePtr tmp = NULL;	//pomocny ukazatel
	if((*RootPtr) == NULL){	//ak je strom prazdny treba vytvorit koprenovy uzol
		...
		tmp = (*RootPtr);
		while((tmp->Cont) != Content){	//kym nenajdeme hladany obsah
			if((tmp->Cont) > Content){	//ak je obsah uzlu mensi ako hladany => vlavo
				if((tmp->LPtr) == NULL){
			...
			else if((tmp->Cont) < Content){	//ak je obsah uzlu vacsi ako hladany => vpravo
				if((tmp->RPtr) == NULL){
...
**/
	...
	while(!SEmptyP(&stack)){	//kym je nieco na zasobniku
...
**/
	...
	while(!SEmptyP(&stackP)){
		...
		if(left){		//prichadza zlava, pojde doprava
			SPushB(&stackB,FALSE);
			Leftmost_Postorder(RootPtr->RPtr,&stackP,&stackB);
		}
		else{			//zprava, odstrani a spracuje uzol
			STopPopP(&stackP);
			BTWorkOut(RootPtr);
		}
	}
...
**/
	tStackP stack;
	SInitP(&stack);
	tBTNodePtr ptr = NULL;		//pomocny ukazatel
	do{
		//ak sme narazili na prazdny uzol a je prazdny zasobnik potom zoberieme ukazatel 
		//zo zasobnika
		if((*RootPtr) == NULL){
			if(!SEmptyP(&stack))
				(*RootPtr) = STopPopP(&stack);
			...
			if(((*RootPtr)->RPtr) != NULL)	//ak uzol uz nema pravy ukazatel dame ho 
							//na zasobnik
				SPushP(&stack,(*RootPtr)->RPtr);
			ptr = (*RootPtr);		//odstranime uzol
